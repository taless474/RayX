// exp69 -- standalone EXPERIMENT-ONLY island root for the same-axis top-k performance probe.
//
...
//   actor A + actor B), and finalizes on the root.done completion sentinel -- the exp63/64/65/66/
//   67/68 user-space lifecycle protocol. On root.done it does NOT finalize immediately: it keeps
...
//
// exp69 has NO prober role: the two Ray-actor localities are the top-k endpoints for BOTH
// comparison arms, and the ONLY timed boundary is the controller's perf_counter_ns around one
// ray.get. This binary is never on a timed path.
//
// Registers the exp69 actions (ONE TU) so the root binary matches the island's action table.
...
#include "topk_action.hpp"  // ONE TU per binary: registers exp69 topk/pid actions
...
int root_hpx_main(hpx::program_options::variables_map&) {
    ...
    write_text(g_bootdir + "/root.final",
               "{" + stamp("root") + ",\"max_membership\":" + std::to_string(max_membership) +
                   ...
                   ",\"actions_served_on_root\":" +
                   std::to_string(exp69_action_serve_count.load()) + "}\n");
    return hpx::finalize();
}
...
int main(int argc, char* argv[]) {
    ...
    po::options_description desc("exp69_peer options");
    desc.add_options()
    ...
    if (role != "root") {
        std::fprintf(stderr, "exp69_peer: unsupported role '%s' (only 'root')\n", role.c_str());
        return 2;
    }
