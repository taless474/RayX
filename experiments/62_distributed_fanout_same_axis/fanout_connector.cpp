// exp62 fanout_connector -- EXPERIMENT-ONLY standalone HPX connect-mode locality for the Slice 2
// one-remote-locality fanout dry-run.
//
// ROLE: this binary is the REMOTE locality (node B). The exp62 ROOT is the embedded HPX runtime
// inside the Python process (fanout_ext), NOT a binary. This connector joins the root's AGAS in
// runtime_mode::connect, registers the SAME exp62_leaf_action (via fanout_action.hpp, so the action
// name matches the root's), and stays alive as a served remote locality the root dispatches N leaf
// actions to. It does NO timing of its own: the single measurement clock lives on the Python caller
// side, exactly like exp61's connector.
//
// It is a copy-in-spirit of the exp61/exp58/exp57 connector role (connect-mode start, optional
// bounded AGAS TCP pre-probe, shared-FS attestation + rendezvous, post(disconnect)+stop clean
// leave, Linux-only getsockopt(TCP_NODELAY) + sched_getaffinity attestation). It is NOT linked into
...
//
// CLAIM FENCE: Slice 2 MVP mechanism only. Being a reachable remote locality is a MECHANISM, not a
...
#ifndef EXP62_BUILD_TYPE
#define EXP62_BUILD_TYPE "unknown"
#endif
#ifndef EXP62_COMPILER_ID
#define EXP62_COMPILER_ID "unknown"
#endif
#ifndef EXP62_COMPILER_VERSION
#define EXP62_COMPILER_VERSION "unknown"
#endif
...
// Those are the actual connections to the root locality's parcelport. FAIL-CLOSED on non-Linux, no
// match, or getsockopt failure. We never fabricate a "verified" value.
...
int run_connector(int argc, char** argv,
    ...
    auto write_attest = [&](const nodelay_attestation& nd) {
        ...
        j += "\"action_registration_name\":\"exp62_leaf_action\",";
...
int run_probe_info(int argc, char** argv) {
    ...
    j += "\"action_registration_name\":\"exp62_leaf_action\",";
    j += "\"build_type\":\"" EXP62_BUILD_TYPE "\",";
    j += "\"compiler_id\":\"" EXP62_COMPILER_ID "\",";
    j += "\"compiler_version\":\"" EXP62_COMPILER_VERSION "\",";
    j += "\"hpx_started\":false}\n";
...
int main(int argc, char* argv[]) {
    ...
    po::options_description desc("exp62 fanout connector (remote locality) options");
    // clang-format off
    desc.add_options()
