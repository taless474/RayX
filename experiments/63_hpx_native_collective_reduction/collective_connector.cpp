// exp63 collective_connector -- EXPERIMENT-ONLY standalone HPX connect-mode locality for the Slice 1b
// HPX progress root-cause micro-slice.
//
// COPIED/RENAMED from exp62 fanout_connector.cpp (exp62 -> exp63, exp62_leaf_action ->
// exp63_leaf_action, EXP62_* -> EXP63_*). Self-contained under exp63; includes NO exp62 header.
//
// ROLE: this binary is a REMOTE locality (node B / node C). The exp63 ROOT is the embedded HPX
// runtime inside the Python process (collective_ext), NOT a binary. This connector joins the root's
// AGAS in runtime_mode::connect, registers the SAME exp63_leaf_action (via collective_action.hpp),
// and stays alive as a served remote locality the root dispatches N leaf actions to. It does NO
// timing of its own: the single measurement clock lives on the Python caller side.
//
// It is a copy-in-spirit of the exp61/exp62 connector role (connect-mode start, optional bounded AGAS
// TCP pre-probe, shared-FS attestation + rendezvous, post(disconnect)+stop clean leave, Linux-only
// getsockopt(TCP_NODELAY) + sched_getaffinity attestation). It is NOT linked into
// python/CMakeLists.txt, links no RayX/production code, has NO Ray anywhere, and adds NO distributed
// action to any shipped rayx.runtime API.
//
// CLAIM FENCE: Slice 1b mechanism/progress only. Being a reachable remote locality is a MECHANISM,
// not a performance result. No same-axis comparison, no speedup, no ratio, no "HPX beats Ray". The
...
#ifndef EXP63_BUILD_TYPE
#define EXP63_BUILD_TYPE "unknown"
#endif
#ifndef EXP63_COMPILER_ID
#define EXP63_COMPILER_ID "unknown"
#endif
#ifndef EXP63_COMPILER_VERSION
#define EXP63_COMPILER_VERSION "unknown"
#endif
...
// Connector-lifetime hardening (exp63): unix seconds now, and a tiny double reader for the root's
// completion-time stamp. Used only for provenance timestamps, never for the deadman comparison (which
// is done on the connector's own steady clock to stay robust to cross-node wall-clock skew).
double now_unix() {
...
// connector runs on the remote node, so this is node B's own IP -- NOT the root IP. Empty when
// nothing matches (then we do not inject --hpx:hpx).
...
int run_connector(int argc, char** argv,
    ...
    auto write_attest = [&](const nodelay_attestation& nd) {
        ...
        j += "\"action_registration_name\":\"exp63_leaf_action\",";
    ...
    // CONNECTOR-LIFETIME HARDENING (exp63): stay alive until the ROOT signals COMPLETION (root.done,
    // or the legacy served1.ok), NOT until a fixed serve-timeout. serve-timeout is now a DEADMAN guard
    // that fires ONLY if the root goes SILENT -- i.e. its heartbeat file (root.alive, whose mtime the
    // root bumps before every dispatch) has not advanced for serve_timeout seconds, measured on the
    // connector's OWN steady clock (robust to cross-node wall-clock skew). This is why a low
    // serve-timeout can now cover a long run: as long as the root keeps dispatching (heartbeating), the
    // deadman never fires, and the connector leaves only when the root says it is done. If the runner
    // is an OLD build that never heartbeats, the deadman degrades to the previous fixed-timeout behavior.
    ...
    // serve-end: the root dispatched the leaf actions during timing, so the parcelport peer
    // connection is warm -- sample the REAL TCP_NODELAY on it now and overwrite the attestation.
    ...
    write_text(bootdir + "/connect.disconnected1",
               "{\"clean\":" + bquote(clean) + ",\"rc\":" + std::to_string(rc) +
                   ...
                   ",\"build_type\":\"" EXP63_BUILD_TYPE "\"" +
                   ",\"error\":\"" + json_escape(err) + "\"}\n");
    return clean ? 0 : 1;
}

// --probe-info: print hostname + advertised endpoint WITHOUT starting HPX or networking, so the
// runner can sanity-check the binary off-cluster (e.g. local) without a two-node allocation.
int run_probe_info(int argc, char** argv) {
    ...
    j += "\"action_registration_name\":\"exp63_leaf_action\",";
    j += "\"build_type\":\"" EXP63_BUILD_TYPE "\",";
    j += "\"compiler_id\":\"" EXP63_COMPILER_ID "\",";
    j += "\"compiler_version\":\"" EXP63_COMPILER_VERSION "\",";
    j += "\"hpx_started\":false}\n";
...
int main(int argc, char* argv[]) {
    ...
    po::options_description desc("exp63 collective connector (remote locality) options");
    // clang-format off
    desc.add_options()
