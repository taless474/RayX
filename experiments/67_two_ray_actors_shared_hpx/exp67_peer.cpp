// exp67 -- standalone EXPERIMENT-ONLY island root for the two-Ray-actors-share-one-HPX probe.
//
// ONE role: --role root  WORK-FREE island root (console mode / AGAS root / locality 0),
//   separately supervised by the Python controller. It hosts NO application work: its hpx_main
//   only heartbeats root.alive, observes membership counts (introspection, not work; expect it
//   to reach 3 = root + actor A + actor B), and finalizes on the root.done completion sentinel --
//   the exp63/64/65/66 user-space lifecycle protocol. On root.done the root does NOT finalize
//   immediately: it keeps heartbeating and waits (bounded) for EVERY connector to leave gracefully
//   (membership back to 1), so finalize never races an actor's post(disconnect)+stop. The
//   controller verifies BY VALUE that no probe/oracle action ever executed on locality 0.
//
// exp67 has NO prober role: the two Ray-actor localities are themselves the action endpoints, and
// the load-bearing proof is actor-to-actor (A<->B), driven from the actors' in-process runtimes.
//
// This experiment REQUIRES the verified waiter-fix HPX build (exp64 Slice 5V); the runtime-observed
// HPX identity is written into every marker so the controller can assert it. Durations are
// observational only. NOT production code, NOT rayx.runtime API, no performance/ratio/winner claim.
...
int main(int argc, char* argv[]) {
    ...
    po::options_description desc("exp67_peer options");
    desc.add_options()
    ...
    if (role != "root") {
        // exp67 supports only the work-free root here; actor endpoints host their own runtimes.
        std::fprintf(stderr, "exp67_peer: unsupported role '%s' (only 'root')\n", role.c_str());
        return 2;
    }
