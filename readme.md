...
* The experimental **`rayx.endpoint`** seam: minted endpoint identities, a local
  AF_UNIX endpoint transport, and a private endpoint→Runtime bridge. Ray/Python
  carries bootstrap metadata; the endpoint layer is **HPX-free** (the `Runtime`
  owns HPX). Local cross-process plumbing only — not HPX transport/serving, not a
  fabric, not parcelport / AGAS, not multi-node, and no public endpoint-call API
  beyond the existing fixed ping.
* The **in-process composition evidence arc** (exp39–44): repeated Python-mediated
  dependent steps pay a boundary-crossing cost that scales with chain length
  (exp39), HPX-native fan-out can live behind one `Runtime` op (exp40), HPX
  cooperative interleaving is real (exp41), and the **exp44 keystone** shows the
  Runtime boundary and lane execution preserve cooperative HPX scheduling for a
  barrier-gated fan-in interior at one observed HPX OS worker. Structural/mechanism
  evidence on a synthetic, local, narrow runtime — not performance evidence; the
  future distributed-fabric direction remains gated.
...
parked-wait diagnostic work; no performance claim) / the exp39 dependent-chain
pair `chain_sum_loop` (plain native loop) and `chain_sum_then` (`hpx::future::then`
continuation chain; synthetic decomposition diagnostic, no performance claim) /
the exp40 intra-op `chain_fanout` (independent `hpx::async` children + `when_all`
fold) / the exp44 diagnostic `barrier_fanin` (a **barrier-gated fan-in** that
returns a closed `int64` plus an opt-in **debug-only structural witness** side
channel via `Runtime.barrier_fanin_witness()`; used for exp44 structural
validation — structural/mechanism evidence only, no speedup / throughput / latency
/ parallelism / Ray / fabric claim), and `counter` actor methods
...
`Runtime.barrier_fanin_witness()` returns a mutex-guarded structural snapshot
(debug/test only, single-in-flight; stale/cross-call reads are possible but not
torn reads) — not scheduler state, not placement control, not a synchronization
primitive, and not a public scheduler API; its `max_simultaneously_suspended_leaves`
field is **coordinated cooperative suspension, not parallelism**.
...
### Experimental: `rayx.endpoint` seam + A1 local transport (design notes)

A separate, **exploratory** distributed-direction workstream, still narrow and honest.
`rayx.endpoint` now has an opt-in **local endpoint transport**: Ray/Python exchanges
endpoint metadata for bootstrap, and `Endpoint(transport=True)` can deliver one fixed
typed peer-specific `int64` ping across two local actor processes over **AF_UNIX IPC**.
This proves **local cross-process endpoint delivery only**. It is plain native IPC —
**not** HPX transport, **not** HPX serving, **not** a fabric, **not** parcelport / AGAS,
**not** multi-node — and carries **no performance claim**. Transport is off by default
(a cross-process peer is then cleanly `EndpointUnreachableError`), the ping is computed
inline with no HPX call in the data path, and there is no object store / `ObjectRef`, no
arbitrary payload, and no Python callback.

* [docs/design/endpoint_runtime_seam.md](docs/design/endpoint_runtime_seam.md) — the **canonical, consolidated** endpoint design narrative, readable end to end: **endpoint identity** (minted `rtb-ep-<16 hex>` ids, process-local registry, the peer-specific typed `int64` handshake, closed/tombstone behavior, typed error taxonomy) → **A1 local transport** (opt-in `Endpoint(transport=True)`, one process-local AF_UNIX listener, owner-only socket dir, the fixed 39B/16B PING frame, one-shot dial-per-call probed-handle semantics, the invariance property, A1.1 robustness hardening) → **shared HPX owner, Variant 2** (`Engine` exclusive, `Runtime` singleton/HPX-owner, `Endpoint` multiple/HPX-free; order-independent teardown; `_process_hpx_active()` is a diagnostic gate only) → **endpoint→Runtime bridge** (private/test-only `Connection._call_op`; fixed closed `CALL_OP` frame; v1 `square`/`add`, v2 composed-op `fanout_sum`; the drain gate; plain-`std::thread` accept thread; parts-invariance is correctness, **not** HPX evidence) → **corrected exp42/exp43 path interpretation**. The endpoint layer is HPX-free, the `Runtime` owns HPX, the bridge is local structural plumbing — **not** HPX socket serving, **not** HPX async socket I/O, **not** a fabric, **not** parcelport / AGAS, **not** multi-node, no persistent transport, and no public endpoint-call API beyond the existing fixed ping. *(This supersedes four earlier notes — `endpoint_identity_seam.md`, `endpoint_transport_slice.md`, `endpoint_runtime_shared_owner.md`, `endpoint_runtime_bridge.md` — now kept as pointer stubs.)*

In short: the endpoint layer is intentionally **HPX-free**, the **`Runtime` owns HPX**, and the endpoint→Runtime bridge is **local structural plumbing**. The local AF_UNIX / endpoint path results below (exp42, exp43) are **observation-only**. There is no HPX socket serving, no HPX async socket I/O, no parcelport, no AGAS, no multi-node, no persistent-transport claim, and no public endpoint-call API beyond the current narrow / private / testing surfaces. The future distributed-fabric direction stays **gated** and is **not** advanced by this endpoint/IPC evidence.

* [experiments/42_endpoint_bridge_boundary_cost/bridge_boundary_cost.md](experiments/42_endpoint_bridge_boundary_cost/bridge_boundary_cost.md) — exp42: local **observation-only** characterization of the endpoint→Runtime path around an existing composed Runtime op. P0 is a direct Runtime submit; **P1 is a same-process in-process bridge dispatch** (same-pid metadata short-circuits through the in-process registry — no socket, no accept thread, no frame codec); **P2 is a cross-process local endpoint bridge** over AF_UNIX into a child Runtime. Learned that `P1−P0` is the in-process bridge-dispatch path difference (**not** socket cost) while `P2−P1` bundles AF_UNIX delivery, the accept-thread hop, a second process, and a second HPX runtime (it does **not** isolate transport). Observation-only and machine-specific — no HPX mechanism/scheduling/value, no speedup/throughput/latency, no Ray comparison, no isolated socket/transport cost, no fabric / parcelport / AGAS / multi-node.
* [experiments/43_endpoint_transport_ping_floor/endpoint_ping_floor.md](experiments/43_endpoint_transport_ping_floor/endpoint_ping_floor.md) — exp43: a **Runtime-less, HPX-free** endpoint ping microprobe (no Runtime in either process; `_process_hpx_active()` is false on both sides). EP0 is a same-process inline ping; **EP1 is a cross-process one-shot dial-per-call AF_UNIX endpoint ping**; EPraw is a same-shape **Python** AF_UNIX echo control. Learned that EP1 is an *undifferentiated end-to-end* one-shot endpoint ping floor, and that EPraw is **not** a raw OS lower bound (its interpreted-Python server is not comparable to EP1's native accept thread), so `EP1−EPraw` is a cross-implementation observation only. Observation-only and OS-local (non-transferable); **not** an OS-floor or fabric claim — no parcelport / AGAS / multi-node, no persistent transport, and no mechanical subtraction from exp42 P2.
...
### In-process HPX composition evidence

The strengthened in-process story: **boundary reduction plus HPX-faithful native
composition behind one coarse Python/Runtime boundary.** exp39 exposes that repeated
Python-mediated dependent steps pay a boundary-crossing cost that scales with chain
length; exp40 shows HPX-native fan-out can live behind one `Runtime` op; exp41 is a
standalone HPX cooperative interleaving / suspension / resume witness; and exp44
integrates these behind one `Runtime` boundary — the **keystone**. All are
structural / mechanism evidence on a synthetic, local runtime: no speedup, no
throughput, no latency, no Ray comparison, no parallelism, and no fabric claim. The
in-process story is strengthened after exp44; the future distributed-fabric direction
remains **gated** and is **not** pulled forward by this in-process or by the
endpoint/IPC evidence.

* [experiments/39_native_continuation_vs_mediated_chain/native_continuation_vs_mediated_chain.md](experiments/39_native_continuation_vs_mediated_chain/native_continuation_vs_mediated_chain.md) — exp39: native loop vs scheduled `future::then` vs Python-mediated chain. Shows that for dependent synthetic native work inside one RayX runtime, repeated Python/pybind/RuntimeFuture/lane boundary cost scales strongly with chain length, while scheduled `future::then` continuation/shared-state overhead is visible but much smaller. This supports the in-process native-composition thesis only; it is not a Ray, multi-node, LLM, Python-callback, or HPX-fabric result.
* [experiments/40_native_independent_overlap/native_independent_overlap.md](experiments/40_native_independent_overlap/native_independent_overlap.md) — exp40: independent-chain overlap, RayX lane-level (Arm A, baseline/control) vs HPX-native intra-op `chain_fanout` (Arm B, load-bearing). Shows on Rostam (structural gates **PASS** 74/74) that Arm B overlaps independent native work inside one RayX runtime — near-ideal up to ~4 effective workers, saturating below ideal at 8 (a characterized K=8/`T=4→T=8` ceiling, not a failure) — and is stronger than Arm A across the matrix. Fixed-core overlap-mechanism result only; binding is HPX default and placement unknown, so no NUMA/socket claim; it is not a Ray-mediated, multi-node, Ray-transport, HPX-fabric, LLM, Python-callback, no-GIL, or "HPX beats Ray" result.
* [experiments/41_barrier_witness_spike/barrier_witness_spike.md](experiments/41_barrier_witness_spike/barrier_witness_spike.md) — exp41: a standalone native HPX **cooperative-interleaving witness** (a shared cooperative gate; multiple HPX tasks suspend mid-execution, the scheduler runs others, the last arriver opens the gate, all resume and complete). Learned that this is load-bearing as a clean pass at `--hpx:threads=1` (with one OS worker, a non-cooperative gate wait would pin the worker and deadlock), with forced-failure signatures (skip / throw / launch-fewer) and an external subprocess timeout as the true anti-hang guard. Cooperative interleaving / suspension / resume only — no parallelism, no multi-worker requirement, no speedup / throughput / latency, no endpoint bridge, no Runtime op, no HPX socket serving, no parcelport / AGAS / multi-node. It is the canonical cooperative-interleaving artifact (deliberately **not** promoted into a Runtime op, so the structural JSON evidence is preserved).
* [experiments/44_barrier_fanin_witness/barrier_fanin_witness.md](experiments/44_barrier_fanin_witness/barrier_fanin_witness.md) — exp44: the **witnessed barrier-gated fan-in keystone**, integrating exp39/40/41 behind one Runtime boundary. exp44 shows the RayX Runtime boundary and lane execution model preserve cooperative HPX scheduling for a barrier-gated fan-in interior: one coarse registered Runtime operation launches bare `hpx::async` leaves, mutually gates them, joins with `when_all`, reduces with a scheduled continuation, and completes at `--hpx:threads=1` with one observed HPX OS worker and structural witness gates passing (`arrived==released==leaves`, `opener==last_arriver`, `watchdog_opened==false`, `ordering_violations==0`, `reduction_after_all_leaves==true`); a non-cooperative interior would deadlock at one worker. This is **structural / mechanism evidence, not performance evidence, not a Ray comparison, not parallelism evidence, and not fabric evidence** — the value oracle (`barrier_fanin(seed, leaves, quantum) == chain_fanout(seed, leaves, 1, quantum)`) is correctness only, and `barrier_fanin` is a diagnostic side-effecting op whose debug-only witness leaves `OpOutcome` / `OperationResult` / `lane_stats()` / the v1 JSONL schema unchanged.

### Ray-free HPX connect-mode lifecycle + Ray-orchestrated bootstrap (experiments 49–52)

A single-node, loopback-TCP, closed-`int64` mechanism/characterization arc that establishes
the **island lifecycle boundary** before any fabric work. These probes set a concrete
**architecture policy**: **Ray is the placement / bootstrap / supervision plane** and **HPX is
the execution / data plane inside one HPX island**; if an HPX locality dies **ungracefully** the
**whole island is treated as poisoned**, and the recovery policy is **whole-island external
restart, not in-place repair of stale AGAS state** — this is **external supervision, not HPX
fault tolerance**. All four are structural; they make **no** fault-tolerance, crash-recovery,
AGAS-root-loss-recovery, multi-node, general-fabric, production/public-API, or
performance/speedup/throughput/latency claim, and the **future distributed-fabric direction
remains gated** and is **not** pulled forward by them.

* [experiments/49_strong_l4_hpx_distributed_spike/strong_l4_hpx_distributed_spike.md](experiments/49_strong_l4_hpx_distributed_spike/strong_l4_hpx_distributed_spike.md) — exp49: the Ray-free HPX **connect-mode graceful lifecycle** passes. A root admits connector #1, serves a registered closed-`int64` HPX action on it, the connector self-disconnects via the empirical `post(disconnect)+stop` teardown, the root re-admits and serves connector #2, and finalizes cleanly. Mechanism/lifecycle feasibility only — graceful path, single-node, loopback TCP; no fabric, multi-node, performance, or fault-tolerance claim.
* [experiments/50_strong_l4_connect_failure/strong_l4_connect_failure.md](experiments/50_strong_l4_connect_failure/strong_l4_connect_failure.md) — exp50: **ungraceful** connector loss (SIGKILL) characterized. The root can **still admit and serve a fresh connector** by **set-difference** targeting (robust to AGAS retaining or reusing the dead id), but AGAS **retains the stale dead locality** and the root **hangs at collective finalize** (no self-terminate, no HPX exception enum). The runtime stays usable for a new peer but clean shutdown is poisoned — explicitly **not** fault tolerance.
* [experiments/51_strong_l4_stale_locality_shutdown/strong_l4_stale_locality_shutdown.md](experiments/51_strong_l4_stale_locality_shutdown/strong_l4_stale_locality_shutdown.md) — exp51: the **stale-locality shutdown boundary** characterized. **Bounded finalize** (its timeout drains local threads, not the membership wait) and **local-cache cleanup** (`remove_resolved_locality` / `remove_from_connection_cache`, internal-ish and **not** public AGAS eviction) both **return but do not cure** the hang; a captured backtrace localizes the root to a never-signaled `runtime_distributed::wait()` shutdown-completion wait. **External whole-island restart yields a clean fresh island** — policy, not in-place repair. No public AGAS stale-locality eviction path was found.
* [experiments/52_ray_bootstrap_clean_island/ray_bootstrap_clean_island.md](experiments/52_ray_bootstrap_clean_island/ray_bootstrap_clean_island.md) — exp52: the first **Ray-orchestrated clean bootstrap** passes. Two Ray actors launch the HPX root and connector **child processes**; Ray carries **only bootstrap metadata** (the AGAS endpoint, ports, rendezvous dir, role/index, timeouts), the HPX action travels **HPX → HPX over the parcelport** (never through Ray), and the island tears down via the exp49 graceful path with a clean root finalize. This is **Ray launch/bootstrap/supervision plumbing** (the role `mpirun` / `srun` / `hpxrun.py` play), **not** a new HPX mechanism result — from HPX's view it is exp49 launched by a different parent. Clean path only; the whole-island-fatal policy is **assumed, not exercised**.
